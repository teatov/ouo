...
void ouo_tok_dump(OuoToken *tok) {
  ouo_printdbg(
      "[%s '%.*s'] ", _ouo_tok_kind_str(tok->kind), _OUO_TOK_FMT(*tok));
}
#endif // OUO_DEBUG
...
#define _ouo_p_err_unexpected(p, tok, exp) \
  _ouo_p_err(p, tok, OUO_ERR_SYNTAX, "Expected '%s', got '%.*s'.", \
      _ouo_tok_kind_str(exp), _OUO_TOK_FMT(tok))
...
static inline OuoTypeKind _ouo_p_type(_OuoParser *p) {
  switch (p->curr.kind) {
    ...
    case OUO_TOK_KW_STR: return OUO_TYPE_STR;
...
static OuoAst *_ouo_p_expr(_OuoParser *p, _OuoPrecedence prec) {
  ...
  if (prefix_fn == NULL) {
    _ouo_p_err(p, p->curr, OUO_ERR_SYNTAX,
        "Expected an expression, got '%.*s'.", _OUO_TOK_FMT(p->curr));
    return NULL;
  }
  ...
  while (p->peek.kind != OUO_TOK_NEWLINE &&
    ...
    if (infix_fn == NULL) {
      _ouo_p_err(p, p->peek, OUO_ERR_SYNTAX,
          "Expected an operator, got '%.*s'.", _OUO_TOK_FMT(p->peek));
      return left;
    }
...
static OuoAst *_ouo_p_decl_var(_OuoParser *p) {
  ...
  ast->as.decl_var.type_annotation = OUO_TYPE_UNKNOWN;
  ...
  if (p->curr.kind == OUO_TOK_COLON) {
    _ouo_p_advance(p);
    ast->as.decl_var.type_annotation = _ouo_p_type(p);
    if (ast->as.decl_var.type_annotation == OUO_TYPE_UNKNOWN) return ast;
    _ouo_p_advance(p);
  }
...
static OuoAst *_ouo_p_decl_fn(_OuoParser *p) {
  ...
  if (p->curr.kind == OUO_TOK_COLON) {
    _ouo_p_advance(p);
    ast->as.decl_fn.return_type = _ouo_p_type(p);
    if (ast->as.decl_fn.return_type == OUO_TYPE_UNKNOWN) return ast;
    _ouo_p_advance(p);
  }
...
void ouo_ast_free(OuoAst *ast) {
  ...
  switch (ast->kind) {
    ...
    case OUO_AST_PRINT: ouo_ast_free(ast->as.expr_stmt.expr); break;
    case OUO_AST_DECL_VAR: ouo_ast_free(ast->as.decl_var.value); break;
...
void ouo_ast_dump(OuoAst *ast) {
  ...
  switch (ast->kind) {
    ...
    case OUO_AST_BLOCK:
      ...
      break;
    case OUO_AST_IDENT:
      ouo_printdbg("%.*s", _OUO_TOK_FMT(ast->as.ident.name));
      break;
    // Literals
    ...
    case OUO_AST_LIT_STR:
      ouo_printdbg("%.*s", _OUO_STR_FMT(ast->as.lit_str));
      break;
    // Expressions
    case OUO_AST_ASSIGN:
      ouo_ast_dump(ast->as.assign.target);
      ouo_printdbg(" %s ", _ouo_tok_kind_str(OUO_TOK_ASSIGN));
      ouo_ast_dump(ast->as.assign.value);
      break;
    case OUO_AST_BINARY:
      ouo_ast_dump(ast->as.binary.left);
      ouo_printdbg(" %s ", _ouo_tok_kind_str(ast->as.binary.op));
      ouo_ast_dump(ast->as.binary.right);
      break;
    case OUO_AST_UNARY:
    ...
    case OUO_AST_IF:
      ...
      ouo_printdbg(" then ");
      ouo_ast_dump(ast->as.if_expr.then_branch);
      if (ast->as.if_expr.else_branch != NULL) {
        ouo_printdbg(" else ");
        ouo_ast_dump(ast->as.if_expr.else_branch);
      }
      break;
    case OUO_AST_WHILE:
      ouo_ast_dump(ast->as.while_expr.condition);
      ouo_ast_dump(ast->as.while_expr.body);
      break;
    // Statements
    case OUO_AST_EXPR_STMT:
    case OUO_AST_PRINT:
      ouo_printdbg("%d ", ast->as.expr_stmt.pop);
      ouo_ast_dump(ast->as.expr_stmt.expr);
      break;
    case OUO_AST_DECL_VAR:
      ouo_printdbg("%.*s %s ", _OUO_TOK_FMT(ast->as.decl_var.name),
          _ouo_type_kind_str(ast->as.decl_var.type_annotation));
...
static inline bool _ouo_chunk_find_sym(
  ...
  for (size_t i = syms->count - 1; i >= 0; i--) {
    if (_ouo_str_slice_eq(&name->str, &syms->items[i].name.str)) {
...
static inline OuoSymbol *_ouo_c_chunk_add_sym(_OuoCompiler *c,
    OuoChunkSymbols *syms, OuoAst *ast, OuoToken *name, OuoTypeKind type) {
...
static void _ouo_c_err_ident_undefined(_OuoCompiler *c, OuoAst *ast) {
  _ouo_c_err(c, ast->tok, OUO_ERR_SEMANTIC, "Undefined symbol '%.*s'.",
      _OUO_TOK_FMT(ast->as.ident.name));
}
...
static void _ouo_c_err_binary_type(_OuoCompiler *c, OuoAst *ast) {
  _ouo_c_err(c, ast->tok, OUO_ERR_TYPE,
      ...
      _ouo_type_kind_str(ast->as.binary.left->type),
      _ouo_type_kind_str(ast->as.binary.right->type));
}
...
static void _ouo_c_err_unary_type(_OuoCompiler *c, OuoAst *ast) {
  _ouo_c_err(c, ast->tok, OUO_ERR_TYPE, "Operation '%s' does not support '%s'.",
      ...
      _ouo_type_kind_str(ast->as.unary.right->type));
}
...
static void _ouo_c_err_if_condition_type(_OuoCompiler *c, OuoAst *ast) {
  _ouo_c_err(c, ast->as.if_expr.condition->tok, OUO_ERR_TYPE,
      ...
      _ouo_type_kind_str(ast->as.if_expr.condition->type));
}
...
static void _ouo_c_err_stmt_type(
  ...
  _ouo_c_err(c, *tok, OUO_ERR_TYPE, "Type of '%.*s' cannot be '%s'.",
      _OUO_TOK_FMT(*tok), _ouo_type_kind_str(type));
}
...
static void _ouo_c_err_fn_type(_OuoCompiler *c, OuoAst *ast) {
  _ouo_c_err(c, ast->tok, OUO_ERR_TYPE, "Function returns '%s', but got '%s'.",
      _ouo_type_kind_str(ast->as.decl_fn.return_type),
      _ouo_type_kind_str(ast->as.decl_fn.body->type));
}

static inline bool _ouo_ast_binary_is(OuoAst *ast, OuoTypeKind type) {
  return ast->as.binary.left->type == type &&
      ast->as.binary.right->type == type;
}

static inline bool _ouo_ast_unary_is(OuoAst *ast, OuoTypeKind type) {
  return ast->as.unary.right->type == type;
}
...
static void _ouo_c_ast_analyze(_OuoCompiler *c, OuoAst *ast) {
  switch (ast->kind) {
    ...
    case OUO_AST_LIT_STR: ast->type = OUO_TYPE_STR; break;
    ...
    case OUO_AST_IF: {
      ...
      OuoTypeKind else_type = ast->as.if_expr.else_branch != NULL
          ? ast->as.if_expr.else_branch->type
          : OUO_TYPE_VOID;

      if (ast->as.if_expr.then_branch->type == else_type)
    ...
    case OUO_AST_WHILE: {
      ast->type = OUO_TYPE_VOID;
      if (ast->as.while_expr.condition->type != OUO_TYPE_BOOL) {
        bool panic_prev = c->panic_mode;
        c->panic_mode = false;
        _ouo_c_err_if_condition_type(c, ast);
        c->panic_mode = panic_prev;
    ...
    case OUO_AST_PRINT:
      ast->type = OUO_TYPE_VOID;
      if (ast->as.expr_stmt.expr->type == OUO_TYPE_VOID)
    ...
    case OUO_AST_DECL_VAR: {
      ...
      OuoTypeKind value_type = ast->as.decl_var.value->type;
      OuoTypeKind type = ast->as.decl_var.type_annotation != OUO_TYPE_UNKNOWN
          ? ast->as.decl_var.type_annotation
          : value_type;

      if (type == OUO_TYPE_VOID) {
        _ouo_c_err_var_void(c, &ast->tok);
      } else if (value_type != OUO_TYPE_UNKNOWN && value_type != type) {
        _ouo_c_err_assign_type(
    ...
    case OUO_AST_DECL_FN: {
      ...
      OuoTypeKind body_type = ast->as.decl_fn.body->type;
      OuoTypeKind return_type = ast->as.decl_fn.return_type != OUO_TYPE_UNKNOWN
          ? ast->as.decl_fn.return_type
          : body_type;
...
static inline void _ouo_c_emit_loop(
    _OuoCompiler *c, OuoAst *ast, size_t op_idx) {
  size_t jump = c->res->chunk.count - op_idx + 3;
  if (jump > UINT16_MAX) {
    _ouo_c_err(c, ast->tok, OUO_ERR_COMPILE_FAIL,
...
static void _ouo_c_ast_emit(_OuoCompiler *c, OuoAst *ast) {
  switch (ast->kind) {
    ...
    // Statements
    case OUO_AST_EXPR_STMT:
      if (ast->as.expr_stmt.pop && ast->type != OUO_TYPE_VOID) {
...
static void _ouo_c_ast_visit(_OuoCompiler *c, OuoAst *ast) {
  ...
  switch (ast->kind) {
    ...
    case OUO_AST_WHILE: {
#ifndef OUO_NOEMIT
      size_t loop_start = c->res->chunk.count;
#endif
